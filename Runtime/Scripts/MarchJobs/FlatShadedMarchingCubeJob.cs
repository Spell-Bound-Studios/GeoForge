...
namespace Spellbound.GeoForge {
    ...
    /// </summary>
    [BurstCompile]
    internal struct FlatShadedMarchingCubeJob : IJob {
        ...
        public void Execute() {
            ...
            // O(1) flat-shade lookup. Any material index whose entry is true will cause its triangle to be
            // flat-shaded ("any flat material wins"). Built once here and checked in the triangle loop.
            var flatShadedLookUp = new NativeArray<bool>(256, Allocator.Temp);
            flatShadedLookUp[0] = true;
                ...
                for (var z = 0; z < cubesMarchedPerLeaf; z++) {
                    for (var x = 0; x < cubesMarchedPerLeaf; x++) {
                        ...
                        for (var i = 0; i < indexCount; i += 3) {
                            ...
                            var isFlatShaded = flatShadedLookUp[Vertices[ia].FixedColor.r]
                                               || flatShadedLookUp[Vertices[ib].FixedColor.r]
                                               || flatShadedLookUp[Vertices[ic].FixedColor.r];
